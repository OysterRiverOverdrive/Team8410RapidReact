  ...
  /*public double calculateApproachSpeed(double targetDist, int approachAlg) {
    ...
    //if (approachAlg == Constants.DRIVER_ASSIST_APPROACH_ALG_LINEAR) {
     ...
     /* slope = (Constants.DRIVER_ASSIST_MAX_DRIVE_SPEED - Constants.DRIVER_ASSIST_MIN_DRIVE_SPEED) /
      ...
      // and
      // min speed. This drops the speed off quickly initially, and slowly approaches
      // the stop point.
      //
      // A standard parabola is:
      // y = x^2
      // The vertex of the parabola can be translated left/right and up/down by (h, k)
      // respectively with:
      // y = a(x - h)^2 + k
      //
      // We'll shift the parabola over to the caution distance and max speed with h =
      // x1 and k = y1.
      // y = slope (x - x1)^2 + y1
      //
      ...
      // The parabola above will go through (x1, y1) and we'll adjust the a (slope)
      // value to curve
      // it down to (x2, y2).
      //
      // y = slope (x - x1)^2 + y1
     ...
     /* slope = (Constants.DRIVER_ASSIST_MAX_DRIVE_SPEED - Constants.DRIVER_ASSIST_MIN_DRIVE_SPEED) /
      ...
      // and max
      // speed. The speed stays higer longer and quickly drops off when close to the
      // stop
      // distance.
      //
      // A standard parabola is:
      ...
      // The parabola above will go through (x2, y2) and we'll adjust the a (slope)
      // value to curve
      // it down to (x1, y1).
      //
      // y = slope (x - x2)^2 + y2
      ...
      // speed = slope (target distance - x2)^2 + y2
      speed = (slope * Math.pow(targetDist - Constants.DRIVER_ASSIST_CAUTION_DISTANCE, 2))
